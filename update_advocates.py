...
Sources:
  HubSpot  — closed-won deals + current customers, address fallback chain, NA filter
  Intercom — positive CSAT (5★)
  Capterra — 4-5★ reviews (scrape)
  G2       — 4-5★ reviews (scrape)
  Software Advice / GetApp — 4-5★ reviews (scrape)
  Reddit   — positive mentions (API)
  Google   — web mentions (Custom Search API) + business reviews (Places API)

Verification rule: ≥1 positive signal from any source = on the map.
...
SIGNAL_LABELS = {
    "hs_testimonial":    "Article / testimonial",
    ...
    "capterra_positive": "Capterra (4–5★)",
...
NEGATIVE_KW = ["not happy","unhappy","at risk","churn","cancel","triple check",
               "do not contact","rocky","leaving","switching away","terrible","awful"]
POSITIVE_KW = ["great","love","loves","happy","excellent","recommend","advocate",
...
HS_PROPS = [
    ...
    "media_testimonials_dsp","internal_comments","hs_current_customer",
]
...
def hs_address_with_fallback(hs_id: str, props: dict) -> str:
    """Company address → associated contacts → empty string."""
    addr = build_address(props)
    if addr and len(addr) > 8:
        return addr
    # Fallback: check first 3 associated contacts
    try:
        r = requests.get(
            ...
            for c in r.json().get("results", [])[:3]:
                ...
                if cr.status_code == 200:
                    ...
                    if caddr and len(caddr) > 8:
                        print(f"    Address from contact fallback: {caddr}")
                        return caddr
...
def hs_get_closedwon_company_ids() -> set:
    """Return HubSpot company IDs associated with any closed-won deal."""
    company_ids, after = set(), None
    # Try hs_is_closed_won first; fall back to dealstage text value
    for filter_obj in [
        ...
        while True:
            ...
            if not after:
                break
        if company_ids:
            break  # got results from first filter, no need to try second
...
def hs_get_customers() -> list[dict]:
    ...
    all_companies = list(results_by_id.values())
    print(f"  HubSpot total: {len(all_companies)} companies before NA filter")
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if convos:
                    ...
                    print(f"  Intercom: {len(results)} CSAT records")
                    break
        except Exception as e:
            print(f"  Intercom attempt failed: {e}")
    if not results:
        try:
...
def _ic_extract(convo, headers, results):
    ...
    if contacts:
        try:
            cr = requests.get(f"https://api.intercom.io/contacts/{contacts[0]['id']}", headers=headers, timeout=8)
            if cr.status_code == 200:
                cdata = cr.json()
                cos = cdata.get("companies",{}).get("data",[])
                name = cos[0].get("name","") if cos else (cdata.get("name","") or "")
        except Exception:
            pass
    if name:
...
def _scrape_reviews(url: str, signal_key: str, card_sel: list, rating_sel: list,
                    body_sel: list, reviewer_sel: list, min_rating: float = 4.0) -> list[dict]:
    """Generic review scraper used by Capterra, G2, Software Advice, GetApp."""
    if not BS4:
        return []
    results = []
    try:
...
def scrape_g2() -> list[dict]:
    return _scrape_reviews(
        "https://www.g2.com/products/digitail/reviews",
        "g2_positive",
        ["[itemprop='review']","[class*='Paper__StyledPaper']","[class*='paper--box']","article"],
        ["[itemprop='ratingValue']","[class*='stars']","[aria-label*='star']"],
        ["[itemprop='reviewBody']","[class*='formatted-text']","p"],
        ["[itemprop='author']","[class*='reviewer']","[class*='author']"],
    )

def scrape_software_advice() -> list[dict]:
    return _scrape_reviews(
        "https://www.softwareadvice.com/veterinary/digitail-profile/reviews/",
        "softwareadvice",
...
def scrape_getapp() -> list[dict]:
    return _scrape_reviews(
        "https://www.getapp.com/veterinary-practice-management-software/a/digitail/reviews/",
        "getapp",
...
def scrape_trustpilot() -> list[dict]:
    return _scrape_reviews(
        "https://www.trustpilot.com/review/digitail.io",
        "capterra_positive",  # reuse signal key — same intent
...
def fetch_reddit_mentions() -> list[dict]:
    ...
    try:
        ...
        tok = requests.post(
            ...
            headers={"User-Agent":"DigitailAdvocateMap/3.0"}, timeout=10,
        ).json().get("access_token","")
        if not tok:
            print("  Reddit: auth failed")
            return []
        headers = {"Authorization":f"bearer {tok}","User-Agent":"DigitailAdvocateMap/3.0"}
            ...
            if r.status_code == 200:
                for post in r.json().get("data",{}).get("children",[]):
                    ...
                    if any(p in tl for p in POSITIVE_KW) and not any(n in tl for n in NEGATIVE_KW):
                        results.append({
                            "source":"reddit","text":text[:400],"author":d.get("author",""),
...
def fetch_google_reviews() -> list[dict]:
    """Pull reviews from Digitail's Google Business listing."""
    ...
    try:
        ...
        if r.status_code == 200:
            for review in r.json().get("result",{}).get("reviews",[]):
                if review.get("rating",0) >= 4 and review.get("text",""):
                    results.append({
                        "source":"google","reviewer":review.get("author_name",""),
                        "text":review["text"][:400],"rating":review["rating"],"signal":"google_review",
                    })
...
def fetch_google_web_mentions() -> list[dict]:
    """Use Google Custom Search to find public web mentions of Digitail."""
    if not GOOGLE_KEY or not GOOGLE_CSE_ID:
        print("  Google CSE: no credentials, skipping")
        return []
    results = []
    queries = ['"Digitail" veterinary review', '"Digitail" PIMS "switched"', '"Digitail" vet "recommend"']
    try:
            ...
            if r.status_code == 200:
                for item in r.json().get("items",[]):
                    ...
                    if any(p in sl for p in POSITIVE_KW) and not any(n in sl for n in NEGATIVE_KW):
                        results.append({
                            "source":"google_web","text":snippet[:400],
                            "url":item.get("link",""),"signal":"google_mention",
                        })
            time.sleep(0.3)
        print(f"  Google CSE: {len(results)} positive web mentions")
    except Exception as e:
        print(f"  Google CSE failed: {e}")
    return results

def fetch_facebook_reviews() -> list[dict]:
    """Pull Digitail Facebook page reviews via Graph API."""
    if not FB_TOKEN:
    ...
    try:
        ...
        if r.status_code == 200:
            for review in r.json().get("data",[]):
                if review.get("rating",0) >= 4 and review.get("review_text",""):
                    results.append({
                        "source":"facebook","reviewer":review.get("reviewer",{}).get("name",""),
                        "text":review["review_text"][:400],"rating":review["rating"],"signal":"facebook_review",
                    })
...
def match_to_hs(name: str, companies: dict) -> str | None:
    """Try to find a HubSpot company ID that matches a name string."""
    for hs_id, data in companies.items():
        hs_name = (data.get("properties",{}).get("name") or "").strip()
        if names_match(name, hs_name):
            return hs_id
    return None
...
def main():
    ...
    # Index by ID for name-matching external signals
    ...
    for customer in hs_customers:
        ...
        if not signals:
            continue  # No verified signal — skip
        ...
        # Quote — prefer Intercom > external reviews > existing
        for k, v in ic_sigs.items():
            if names_match(name, k) and v.get("quote"):
                rec["quote"] = v["quote"]
                break
    ...
    # ── Log unmatched external mentions for manual review ─────────────────────
    unmatched = []
    for ext in all_external:
        reviewer = ext.get("reviewer","") or ext.get("author","") or ""
        mid = match_to_hs(reviewer, hs_by_id) or match_to_hs(ext.get("text",""), hs_by_id)
        if not mid and reviewer:
            unmatched.append(f"[{ext['source']}] {reviewer}: {ext.get('text','')[:80]}")
    if unmatched:
        print(f"\n  ⚠️  {len(unmatched)} unmatched external mentions (review manually):")
    ...
    if SLACK_URL:
        ...
        if unmatched:
            lines.append(f"⚠️ *{len(unmatched)} unmatched web mentions* — check Actions log")
